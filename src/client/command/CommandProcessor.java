...
public class CommandProcessor {
    ...
    public static boolean processCommand(MapleClient c, String[] split, char heading) {
        ...
        switch (heading) {
            case '!':
                if (MapleGroups.INTERN.atleast(player)) {
                    if (!InternCmd.executeCommand(c, split))
                        if (MapleGroups.GM.atleast(player))
                                if(!GMCmd.executeCommand(c, split))
                                    if(MapleGroups.SUPER.atleast(player))
                                        if (!SuperCmd.executeCommand(c, split))
                                            if(MapleGroups.ADMIN.atleast(player))
            ...
            case '/':
                if(!PlayerCmd.executeCommand(c, split)){
                    if(MapleGroups.DONATOR.is(player) && DonatorCmd.executeCommand(c, split)) {
                        ;
                    } else if(MapleGroups.VIP.is(player) && VIPCmd.executeCommand(c, split)) {
                        ;
                    } else {
    ...
    public static void executeAdminCommand(MapleClient c, String[] sub, char heading) {
        ...
        if (sub[0].equals("horntail")) {
            ...
            final String query = CommandProcessor.joinStringFrom(sub, 1);
            try {
            ...
            String name = sub[1];
            final String query = CommandProcessor.joinStringFrom(sub, 2);
            try {
                PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement(query);
