...
public class World {
    ...
    short exprate, droprate, mesorate, bossdroprate, nxrate;
    ...
    public World(byte world, byte flag, String eventmsg, short exprate, short droprate, short mesorate, short bossdroprate, short nxrate) {
        this.id = world;
        this.flag = flag;
    ...
    public void setExpRate(short exp) {
        this.exprate = exp;
    }
    ...
    public void setDropRate(short drop) {
        this.droprate = drop;
    }
    ...
    public void setMesoRate(short meso) {
        this.mesorate = meso;
    }
    ...
    public void setNxRate(short nx) {
        nxrate = nx;
    }
    
    public short getNxRate() {
        return nxrate;
    }
    ...
    public void setGuildAndRank(int cid, int guildid, int rank) {
        ...
        if (bDifferentGuild) {
            ...
            if(mc.isGM())
                mc.getMap().broadcastMessage(mc, MaplePacketCreator.spawnPlayerMapobject(mc, false), false);
            else {
                mc.getMap().broadcastGMMessage(mc, MaplePacketCreator.spawnPlayerMapobject(mc, true), false);
                mc.getMap().broadcastNONGMMessage(mc, MaplePacketCreator.spawnPlayerMapobject(mc, false), false);
            }
        }
    }
    ...
    private void updateBuddies(int characterId, byte channel, int[] buddies, boolean offline) {
        ...
        for (int buddy : buddies) {
            MapleCharacter p = playerStorage.getCharacterById(buddy);
            if (p != null) {
                BuddylistEntry ble = p.getBuddylist().get(characterId);
                if (ble != null && ble.isVisible()) {
                    ...
                    if(!chr.isGM() || p.isGM())
                        p.getClient().getSession().write(MaplePacketCreator.updateBuddyChannel(ble.getCharacterId(), mcChannel));
                }
            }
        }
    }
