...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    public void start(MapleClient c, int npc, String filename) {
        try {
            ...
            scripts.put(c, ns);
            //c.announce(MaplePacketCreator.setNPCScriptable(npc, ""));
            if (chr == null) {
                ns.start();
            } else {
                ns.start(chr);
            ...
            dispose(c);
            cms.remove(c);
            notice(c, npc, ute.getMessage());
        } catch (Exception e) {
            System.out.println("Error: NPC " + npc + ".");
            dispose(c);
            cms.remove(c);
            notice(c, npc, e.getMessage());
        }
    }
    ...
    public void action(MapleClient c, byte mode, byte type, int selection) {
        ...
        if (ns != null) {
            try {
                ...
                dispose(c);
                notice(c, getCM(c).getNpc(), ute.getMessage());
            } catch (Exception e) {
                System.out.println("Error: NPC " + getCM(c).getNpc() + ".");
                dispose(c);
                notice(c, getCM(c).getNpc(), e.getMessage());
            }
        }
    }
    ...
    private void notice(MapleClient c, int id, String message) {
        if(c.getPlayer().isGM()){
            c.getPlayer().message(1, "This NPC is not working properly. NPCID: " + id);
            c.getPlayer().message(message + " NPCID: " + id);
        } else {
