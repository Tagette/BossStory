...
public class MapleInventoryManipulator {
    ...
    public static boolean addById(MapleClient c, int itemId, short quantity, String owner, int petid, byte flag, long expiration) {
        ...
        if(itemId == ServerConstants.BRONZE_$_ID
            ...
            c.getPlayer().checkCurrencyConverted();
        }
    ...
    public static boolean addFromDrop(MapleClient c, IItem item, boolean show) {
        ...
        if(item.getItemId() == ServerConstants.BRONZE_$_ID
                || item.getItemId() == ServerConstants.SILVER_$_ID
                || item.getItemId() == ServerConstants.GOLD_$_ID) {
            c.getPlayer().checkCurrencyConverted();
        }
        
        if (ii.isPickupRestricted(item.getItemId()) && c.getPlayer().getItemQuantity(item.getItemId(), true) > 0) {
            System.out.println("You cannot have any more of that item.");
            c.announce(MaplePacketCreator.getInventoryFull());
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            System.out.println("Could not pick up equip with more than 1 quantity.");
            return false;
        }
        if (show) {
    ...
    public static void drop(MapleClient c, MapleInventoryType type, byte src, short quantity) {
        ...
        if (ItemConstants.isPet(source.getItemId())) {
            c.getPlayer().removePet(source.getPet(), true);
            source.getPet().removeFromDb();
        }
        ...
        if (quantity < source.getQuantity() && !ItemConstants.isRechargable(itemId)) {
            ...
            if (weddingRing) {
                ...
                if (ii.isDropRestricted(target.getItemId()) || MapleItemInformationProvider.getInstance().isCash(target.getItemId()) && !ServerConstants.DROPPABLE_CASH) {
                    c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos);
                } else {
                    c.getPlayer().getMap().spawnItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos, true, false);
                }
            } else if (ii.isDropRestricted(target.getItemId()) || MapleItemInformationProvider.getInstance().isCash(target.getItemId()) && !ServerConstants.DROPPABLE_CASH) {
                c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos);
            } else {
            ...
            if (c.getPlayer().getMap().getEverlast()) {
                if (ii.isDropRestricted(itemId) && !ServerConstants.DROPPABLE_RESTRICTED) {
                    c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), source, dropPos);
                } else {
                    c.getPlayer().getMap().spawnItemDrop(c.getPlayer(), c.getPlayer(), source, dropPos, true, false);
                }
            } else if (ii.isDropRestricted(itemId) && !ServerConstants.DROPPABLE_RESTRICTED) {
                c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), source, dropPos);
            } else {
