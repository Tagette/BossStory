...
public class MapleStatEffect {
    ...
    public boolean applyTo(MapleCharacter chr, boolean primary) {
        return applyTo(chr, chr, primary, null);
    }
    ...
    private boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, Point pos) {
        ...
        if (!primary && isResurrection()) {
            ...
            if(applyto.isGM())
                ...
                applyto.getMap().broadcastGMMessage(applyto, MaplePacketCreator.spawnPlayerMapobject(applyto, true), false);
                applyto.getMap().broadcastNONGMMessage(applyto, MaplePacketCreator.spawnPlayerMapobject(applyto, false), false);
            }
        }
...
//	final ScheduledFuture<?> schedule = TimerManager.getInstance().schedule(cancelAction, ((starttime + 99999) - System.currentTimeMillis()));
    ...
    private int calcMPChange(MapleCharacter applyfrom, boolean primary) {
        ...
        if (primary) {
            if (mpCon != 0) {
                double mod = 1.0;
                boolean isAFpMage = applyfrom.getJob().isA(MapleJob.Fire_Poison_Mage);
                boolean isCygnus = applyfrom.getJob().isA(MapleJob.Blaze_Wizard_2);
                if (isAFpMage || isCygnus || applyfrom.getJob().isA(MapleJob.Ice_Lightning_Mage)) {
